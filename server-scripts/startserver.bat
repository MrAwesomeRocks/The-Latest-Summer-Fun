...
:MAIN
java -jar serverstarter-2.1.0.jar
GOTO EOF

:CHECK
REM Check if serverstarter JAR is already downloaded
IF NOT EXIST "%cd%\serverstarter-2.1.0.jar" (
	ECHO serverstarter binary not found, downloading serverstarter...
	%SYSTEMROOT%\SYSTEM32\bitsadmin.exe /rawreturn /nowrap /transfer starter /dynamic /download /priority foreground https://github.com/BloodyMods/ServerStarter/releases/download/v2.1.0/serverstarter-2.1.0.jar "%cd%\serverstarter-2.1.0.jar"
   GOTO MAIN
) ELSE (
   GOTO MAIN
)
